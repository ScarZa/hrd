...
$(function () {
    ...
    $(document).ready(function() {
        chart = new Highcharts.Chart({
            ...
            title: {
                text: ' 5 ลำดับโรคแรกที่พบมากที่สุด'
            },
            subtitle: {
                text: '(ร้อยละ)'
            },
            ...
            tooltip: {
                formatter: function() {
                    return ''+
                        this.series.name +': '+'ร้อยละ '+ this.y +' ';
                }
            },
            plotOptions: {
                bar: {
...
$(function () {
    ...
    $(document).ready(function() {
        chart = new Highcharts.Chart({
            ...
            title: {
                text: 'จำแนกโรคตามภูมิภาค  โรค <?php echo $icd[code]." : ".$icd[name];?>'
            },
            subtitle: {
                text: 'จำนวน ราย'
            },
...
<fieldset data-role="fieldcontain"> 
				...
				echo "<option value='session.php?dx='>ดูรายงานโรคแยกตามภูมิภาค</option>";
 				echo "<option value='session.php?dx=0'>ดูทั้งหมด</option>";
				 while($rs=mysql_fetch_assoc($sql)){
 					 if($dx == $rs['code']){$sel = 'selected';}else{$sel = '';} 
					echo "<option value='session.php?dx=$rs[code]'  $sel   >$rs[code] - $rs[name] : $rs[tname]</option>";
					}
