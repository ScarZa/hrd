...
<body>
 ...
 ?>
		  ...
		  <p><h3>  &nbsp;&nbsp;จำแนกโรคตามตำบล : โรค <?php echo $icd[code]." : ".$icd[name];?></h3></p>
<?php
 ...
 $strQuery = "select t.pt_id  ,t.chwpart,t.amppart,count(d.dx1) as num_dx1  from  pyschosis d,  pt t   where d.dx1='$dx' and t.chwpart='$chwpart'    and  d.pt_id=t.pt_id 	group by   t.amppart  ";
			...
			if($amppart!=''){  
				$strQuery = "select t.pt_id  ,t.chwpart,t.amppart,t.tmbpart,count(d.dx1) as num_dx1  from  pyschosis d,  pt t   where d.dx1='$dx' and t.chwpart='$chwpart'    and  d.pt_id=t.pt_id and t.amppart='$amppart' group by   t.tmbpart  ";
 			  ...
 			  if ($result) {
					  while($ors2 = mysql_fetch_array($result)){
						   $tmbpart =$ors2[tmbpart];
						   $amphur_id= $ors2[amppart];
						   $sql=mysql_query("select name from thaiaddress where amppart='$amppart' and chwpart='$chwpart' and tmbpart='$tmbpart'  ");
 						   $tmb=mysql_fetch_assoc($sql);
						     $name.="'$tmb[name]'".',';
