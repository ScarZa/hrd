...
<CENTER>
<h2>จำนวนผู้ป่วยที่มารับบริการที่ผู้ป่วยนอก ช่วงวันที่ <?php echo  Thaidate($year_now);?>   ถึง <?php echo  ThaiDate($year_now1);?></h2>
<h3>โรงพยาบาลนาโพธิ์ ของปีงบ  <? echo $year_now+543;?></h3>
<?php
...
//    $strQuery = "select * from Factory_Master";	
 //   $result = mysql_query($strQuery) or die(mysql_error());
            $strQuery = "select concat(DATE_FORMAT(vstdate, '%m'),'-',YEAR(vstdate)+543) AS vstdate,count(distinct(hn)) AS count_hn from vn_stat where vstdate between '$year_now' and '$year_now1' GROUP BY MONTH(vstdate)  ORDER BY YEAR(vstdate) ASC";
